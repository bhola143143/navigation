...
open class SwipeRevealLayoutNew : ViewGroup {
    /**
    ...
    class SimpleSwipeListener : SwipeListener {
        override fun onClosed(view: SwipeRevealLayoutNew?) {}
        override fun onOpened(view: SwipeRevealLayoutNew?) {}
        override fun onSlide(view: SwipeRevealLayoutNew?, slideOffset: Float) {}
    }
    ...
    fun open(animation: Boolean) {
        ...
        ViewCompat.postInvalidateOnAnimation(this@SwipeRevealLayoutNew)
    }
    ...
    /** Abort current motion in progress. Only used for {@linko}  */
    fun abort() {
        mAborted = true
        mDragHelper!!.abort()
    }
        ...
        override fun onViewPositionChanged(
            ...
            if (mSwipeListener != null && isMoved) {
                if (mMainView!!.left == mRectMainClose.left && mMainView!!.top == mRectMainClose.top) {
                    mSwipeListener!!.onClosed(this@SwipeRevealLayoutNew)
                } else if (mMainView!!.left == mRectMainOpen.left && mMainView!!.top == mRectMainOpen.top) {
                    mSwipeListener!!.onOpened(this@SwipeRevealLayoutNew)
                } else {
                    mSwipeListener!!.onSlide(this@SwipeRevealLayoutNew, slideOffset)
                }
            }
            mLastMainLeft = mMainView!!.left
            mLastMainTop = mMainView!!.top
